...
- **Web検索（Google検索グラウンディング）**: gemini 系モデルで Gemini ネイティブ経路に
  `google_search` 付きで送信し、応答の下に検索クエリと参照ソース、応答時間を表示
- **公開アプリ認証**: トークンの代わりにアプリ ID を `X-Broker-App` ヘッダで送る接続方式
- **URL ハッシュからのプリフィル**: `/#token=agk_…&model=…` のリンクで接続設定を受け取れる
  （ゲートウェイ管理画面からの遷移用。反映後にハッシュは消去される）
